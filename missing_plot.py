...
def plot_missing(dataframe, title, directory="observed_qc"):
    fig = plt.figure(figsize=(10, 6), dpi=120)
    plt.title("{} Missing Value".format(title))
    dataframe.index = dataframe.index.strftime("%Y-%m")
    sns.heatmap(dataframe, cmap='Spectral_r')
    fig.savefig('./{}/{}_missing_value.png'.format(directory, title), dpi=120)
...
if __name__ == "__main__":
    ...
    for file_name in os.listdir(directory):
        ...
        if df.shape[0] > 0:
            print(file_name)
            plot_missing(df, file_name, directory=output_directory)